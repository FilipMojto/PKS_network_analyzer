...
//	max.push_back(records.begin()->first);
//
...
//
//		if (iterator->second > records.at(max.at(0))) {
//			max.clear();
//			max.push_back(iterator->first);
//		} else if (iterator->second == records.at(max.at(0))) {
//			max.push_back(iterator->first);
//		}
//
...
//	}
//
//	for (const auto& ip : max) {
//		file_stream << "&" << std::endl << ip << std::endl;
//	}
//
//	//while (iterator != end) {
//	//	file_stream << "node: " << iterator->first << std::endl;
...
//	std::cout << "PacketNo: " << packetNo << std::endl;
//
...
//	}
//	else if (protocol_filter->label == "ICMP" or protocol_filter->label == "IPv4") {
...
//
//
//	fileStream << "counts: " << counts << std::endl;
//	
//
//	call_parser_script();
//
...
#define PCAP_FILE_PATH "Protocols\\samples\\z1.pcap"
#define PARSER_FILE_PATH "parser\\packet_parser.py"
#define SCRIPT_INPUT_FILE_PATH "parser\\resources\\input.txt"
#define SCRIPT_OUTPUT_FILE_PATH "parser\\resources\\output.yml"
...
void handlePacket(u_char* temp1, const struct pcap_pkthdr* header, const u_char* pkt_bytes) {
	...
	if (protocol_filter == nullptr or protocol_filter->label == "-count") {
	}
	else if (filter_i == 1 and protocol_filter->label == "TCP") {
		process_request(TCP_comms, dictionary, pkt_bytes, Network::Protocol::TCP);
	}
...
void insert_header_data() {
	...
	if (protocol_filter->label == "-count" or !protocol_filter) {
		fileStream << "packets: " << std::endl;
	}
	else if (protocol_filter) {
		fileStream << "filter_name: " << protocol_filter->label << std::endl;
...
void call_parser_script() {
	...
	else if (protocol_filter->label == "-count") {

		fileStream << IPv4_STAT_DELIMITER << std::endl;
		fileStream << "packet_count: " << counts << std::endl;
		oss << "python " << PARSER_FILE_PATH << " --infile " << SCRIPT_INPUT_FILE_PATH << " --outfile " << SCRIPT_OUTPUT_FILE_PATH << "-c" << std::endl;
	}
	else if (protocol_filter->label == "TCP") {
		process_comms(Network::Protocol::TCP);
		oss << "python " << PARSER_FILE_PATH << " --infile " << SCRIPT_INPUT_FILE_PATH << " --outfile " << SCRIPT_OUTPUT_FILE_PATH << " -p TCP";
